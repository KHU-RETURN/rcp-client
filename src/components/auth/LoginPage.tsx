...
export function LoginPage() {
  ...
  return (
    <AuthLayout headerTitle="Sign in" headerSubtitle="" headerActions={headerActions}>
      <div className="auth-login-box">
        ...
        <div className="roster-group">
          <div className="group-label-row">
            <p className="group-label">가입한 계정</p>
            {customUsers.length > 0 && <span className="muted tiny">{customUsers.length}</span>}
          </div>
          {customUsers.length > 0 ? (
            <div className="roster roster-tight">
              {customUsers.map((user) => (
                <button
                  key={user.id}
                  className="auth-account auth-account-custom"
                  onClick={() => handleUserLogin(user.id)}
                >
                  <strong>{user.name}</strong>
                  <small>@{user.id}</small>
