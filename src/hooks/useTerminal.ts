...
export function useTerminal(
  ...
  const createConsoleSession = useCallback(
    async (inst: Instance): Promise<ConsoleSessionResponse> => {
  ...
  const resolveWebSocketUrl = useCallback((rawUrl: string): string => {
  ...
  const connect = useCallback(
    ...
    [createConsoleSession, resolveWebSocketUrl],
  );
  ...
  }, [instance, hostRef, dispose, connect]);
